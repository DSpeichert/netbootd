...
func (server *Server) tftpReadHandler(filename string, rf io.ReaderFrom) error {
	...
	if mount.Proxy != "" {
		...
		err = tmpl.Execute(buf, mfest.ContentContext{
			...
			HttpBaseUrl: &url.URL{
				Scheme: "http",
				Host:   net.JoinHostPort(laddr.String(), strconv.Itoa(server.hints.HttpPort)),
			},
			ApiBaseUrl: &url.URL{
				Scheme: "http",
				Host:   net.JoinHostPort(laddr.String(), strconv.Itoa(server.hints.ApiPort)),
			},
			SyslogHost: net.JoinHostPort(laddr.String(), strconv.Itoa(server.hints.SyslogPort)),
			Manifest:   manifest,
		})
		if err != nil {
			server.logger.Error().
